...
var eyeLCoords, eyeRCoords, noseCoords, mouthCoords,
  eyeLAngle, eyeLX, eyeLY, eyeRAngle, eyeRX, eyeRY,
  noseAngle, noseX, noseY, mouthAngle, mouthX, mouthY,
  mouthR, chinX, chinY, chinS, faceX, faceY, faceSkew,
  eyebrowSkew, outerEarX, outerEarY, hairX, hairS;

// =======================================================
//  NO MorphSVGPlugin version: swap path "d" instantly
// =======================================================
function swapPathD(targetPathEl, sourcePathEl) {
  if (!targetPathEl || !sourcePathEl) return;
  var d = sourcePathEl.getAttribute("d");
  if (d) targetPathEl.setAttribute("d", d);
}

function setMouthShape(shapeEl) {
  // shapeEl = mouthSmallBG / mouthMediumBG / mouthLargeBG
  swapPathD(mouthBG, shapeEl);
  swapPathD(mouthOutline, shapeEl);
  swapPathD(mouthMaskPath, shapeEl);
}

function setBodyShape(shapeEl) {
  // shapeEl = bodyBGnormal / bodyBGchanged
  swapPathD(bodyBG, shapeEl);
}
...
function onPasswordToggleMouseDown(e) { showPasswordClicked = true; }
function onPasswordToggleMouseUp(e) { showPasswordClicked = false; }
...
function coverEyes() {
  ...
  // No morph: swap body path
  setBodyShape(bodyBGchanged);
...
function uncoverEyes() {
  ...
  TweenMax.to(armR, 1.35, {
    rotation: -105, ease: Quad.easeOut, delay: 0.1, onComplete: function () {
      TweenMax.set([armL, armR], { visibility: "hidden" });
    }
  });

  // Swap back to normal body path
  setBodyShape(document.querySelector('.bodyBGnormal'));
...
function initLoginForm() {
  // Basic null-safety (helps if script loads too early)
  ...
  // Mobile: show password initially
  if (isMobileDevice()) {
    password.type = "text";
